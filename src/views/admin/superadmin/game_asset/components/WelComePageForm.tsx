...
const WelComePageForm = () => {
  ...
  const [isRequired, setIsRequired] = useState<Required>({
    gasAssetName: { gasAssetName: true, fieldName: 'Welcome Image Name' },
    gasAssetImage: { gasAssetImage: true, fieldName: 'Welcome File' },
  });
    ...
    try {
      ...
      if (validationResult.valid) {
        ...
        if (!rowId) {
          ...
          const response = await updateGameAsset(formData, rowId,'welcome');
          setToast({
            enable: true,
            message: response.message,
            status: response.status === 'Success' ? 'success' : 'error',
  ...
  const handleDeleteRow = async (rowData: any) => {
    ...
    try {
      const response = await deleteGameAsset(rowData.gasId,'welcome');
      console.log('response',response);
      if (response.status === "Success") {
        setToast({
          enable: true,
          message: 'Game Asset Welcome Image removed',
          status: 'success',
        });
        ...
        <Grid templateColumns="2fr 2fr 4fr" gap={5}>
          <GridItem w="100%">
            <InputField
              ...
              w="100%"
              id="gasAssetName"
              label="Welcome Image Name"
              isRequired={true}
              placeholder="Welcome Image Name"
              name="gasAssetName"
              onChange={(e: any) => {
                inputHandler(e);
              }}
