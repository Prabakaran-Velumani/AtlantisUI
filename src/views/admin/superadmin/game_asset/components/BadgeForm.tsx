...
const BadgeForm = () => {
  ...
  const [isRequired, setIsRequired] = useState<Required>({
    gasAssetName: { gasAssetName: true, fieldName: 'Badge Image' },
    gasAssetImage: { gasAssetImage: true, fieldName: 'Badge File' },
  });
    ...
    try {
      ...
      if (validationResult.valid) {
        ...
        if (!rowId) {
          ...
          const response = await updateGameAsset(formData, rowId, 'badge');
          setToast({
            enable: true,
            message: response.message,
            status: response.status === 'Success' ? 'success' : 'error',
  ...
  const handleDeleteRow = async (rowData: any) => {
    try {
      const response = await deleteGameAsset(rowData.gasId, 'badge');
      console.log('response', response);
      if (response.status === 'Success') {
        setToast({
          enable: true,
          message: 'Game Asset Badge removed',
          status: 'success',
        });
        ...
        <Grid templateColumns="2fr 2fr 4fr" gap={5}>
          ...
          <GridItem w="100%">
            <FormLabel
              ...
              mb="8px"
            >
              Badge Image Upload<Text color="red">*</Text>
            </FormLabel>
