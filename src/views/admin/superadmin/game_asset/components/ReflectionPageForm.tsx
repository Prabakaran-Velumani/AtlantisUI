...
import ReflectionPageList from './ReflectionPageList';
type Input = {
  gasAssetType: string;
  gasAssetName: string;
...
const ReflectionPageForm = () => {
  ...
  const [isRequired, setIsRequired] = useState<Required>({
    gasAssetName: { gasAssetName: true, fieldName: 'Reflection Name' },
    gasAssetImage: { gasAssetImage: true, fieldName: 'Reflection File' },
  });
    ...
    try {
      ...
      if (validationResult.valid) {
        ...
        if (!rowId) {
          ...
          const response = await updateGameAsset(formData, rowId, 'reflection');
          setToast({
            enable: true,
            message: response.message,
            status: response.status === 'Success' ? 'success' : 'error',
  ...
  const handleDeleteRow = async (rowData: any) => {
    try {
      const response = await deleteGameAsset(rowData.gasId,'reflection');
      console.log('response', response);
      if (response.status === 'Success') {
        setToast({
          enable: true,
          message: 'Game Asset Reflection removed',
          status: 'success',
        });
        ...
        <Grid templateColumns="2fr 2fr 4fr" gap={5}>
          ...
          <GridItem w="100%">
            <FormLabel
              ...
              mb="8px"
            >
              Reflection Image Upload<Text color="red">*</Text>
            </FormLabel>
...
export default ReflectionPageForm;
