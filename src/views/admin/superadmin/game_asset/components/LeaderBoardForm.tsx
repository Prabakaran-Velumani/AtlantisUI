...
const LeaderBoardForm = () => {
  ...
  const [isRequired, setIsRequired] = useState<Required>({
    gasAssetName: { gasAssetName: true, fieldName: 'Leader Board Name' },
    gasAssetImage: { gasAssetImage: true, fieldName: 'LeaderBoard File' },
  });
    ...
    try {
      ...
      if (validationResult.valid) {
        ...
        if (!rowId) {
          ...
          const response = await updateGameAsset(formData, rowId,'leaderboard');
          setToast({
            enable: true,
            message: response.message,
            status: response.status === 'Success' ? 'success' : 'error',
  ...
  const handleDeleteRow = async (rowData: any) => {
    ...
    try {
      const response = await deleteGameAsset(rowData.gasId,'leaderboard');
      console.log('response',response);
      if (response.status === "Success") {
        setToast({
          enable: true,
          message: 'Game Asset LeaderBoard removed',
          status: 'success',
        });
        ...
        <Grid templateColumns="2fr 2fr 4fr" gap={5}>
          <GridItem w="100%">
            <InputField
              ...
              w="100%"
              id="gasAssetName"
              label="Leader Board Name"
              isRequired={true}
              placeholder="Leader Board Name"
              name="gasAssetName"
              onChange={(e: any) => {
                inputHandler(e);
              }}
          ...
          <GridItem w="100%">
            <FormLabel
              ...
              mb="8px"
            >
              Leader Board Image Upload<Text color="red">*</Text>
            </FormLabel>
