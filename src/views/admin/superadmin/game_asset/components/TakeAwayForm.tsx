...
const TakeAwayForm = () => {
  ...
  const [isRequired, setIsRequired] = useState<Required>({
    gasAssetName: { gasAssetName: true, fieldName: 'Take Away Name' },
    gasAssetImage: { gasAssetImage: true, fieldName: 'Take Away Page File' },
  });
    ...
    try {
      ...
      if (validationResult.valid) {
        ...
        if (!rowId) {
          ...
          const response = await updateGameAsset(formData, rowId,'takeaway');
          setToast({
            enable: true,
            message: response.message,
            status: response.status === 'Success' ? 'success' : 'error',
  ...
  const handleDeleteRow = async (rowData: any) => {
    ...
    try {
      const response = await deleteGameAsset(rowData.gasId,'takeaway');
      console.log('response',response);
      if (response.status === "Success") {
        setToast({
          enable: true,
          message: 'Game Asset TakeAway Image removed',
          status: 'success',
        });
        ...
        <Grid templateColumns="2fr 2fr 4fr" gap={5}>
          <GridItem w="100%">
            <InputField
              ...
              w="100%"
              id="gasAssetName"
              label="Take Away Name"
              isRequired={true}
              placeholder="Take Away Name"
              name="gasAssetName"
              onChange={(e: any) => {
                inputHandler(e);
              }}
          ...
          <GridItem w="100%">
            <FormLabel
              ...
              mb="8px"
            >
              Take Away Image Upload<Text color="red">*</Text>
            </FormLabel>
