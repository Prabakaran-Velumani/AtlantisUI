...
import ThankYouPageList from './ThankYouPageList';
...
const ThankYouPageForm = () => {
  ...
  const [isRequired, setIsRequired] = useState<Required>({
    gasAssetName: { gasAssetName: true, fieldName: 'ThankYou Image Name' },
    gasAssetImage: { gasAssetImage: true, fieldName: 'ThankYou File' },
  });
...
console.log('rowId',rowId);
    ...
    try {
      ...
      if (validationResult.valid) {
        ...
        if (!rowId) {
          ...
          const response = await updateGameAsset(formData, rowId,'thankyou');
          setToast({
            enable: true,
            message: response.message,
            status: response.status === 'Success' ? 'success' : 'error',
          ...
          if(response.status === 'Success'){
            ...
            setRowId(null);setIsEditing(false);
          }
  ...
  const handleDeleteRow = async (rowData: any) => {
    ...
    try {
      const response = await deleteGameAsset(rowData.gasId,'thankyou');
      console.log('response',response);
      if (response.status === "Success") {
        setToast({
          enable: true,
          message: 'Game Asset Thank You Image removed',
          status: 'success',
        });
        ...
        <Grid templateColumns="2fr 2fr 4fr" gap={5}>
          <GridItem w="100%">
            <InputField
              ...
              w="100%"
              id="gasAssetName"
              label="ThankYou Image Name"
              isRequired={true}
              placeholder="ThankYou Image Name"
              name="gasAssetName"
              onChange={(e: any) => {
                inputHandler(e);
              }}
          ...
          <GridItem w="100%">
            <FormLabel
              ...
              mb="8px"
            >
              ThankYou Page Image Upload<Text color="red">*</Text>
            </FormLabel>
