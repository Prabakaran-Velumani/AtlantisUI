// Chakra imports
import { AvatarGroup, Avatar, Box, Button, Flex, Icon, Image, Link, Text, useColorModeValue,Tag, TagLabel, TagCloseButton,Select,FormLabel,VStack,IconButton, Badge } from '@chakra-ui/react';
...
export default function NFT(props: {
  ...
  const { image, name, author, tabState, game, handleButtonOne, handleButtonTwo, id,handelDuplicate,handelLaunch,handelAssign,handelMakePublic,handelDelete,gameview, } = props;
...
function formatLikesCount(count:any) {
  ...
  return (
    <Card p="20px"   border={handelAssign.gameNonPlayingCharacterId === id? '2px solid #11047a' : '' || ((game?.gameDuplicated === 'YES' && game?.gameGameStage === 'Creation' && datePart === getCurrentDate()) || (game?.gameDuplicated !== 'YES' && game?.gameGameStage === 'Creation' && datePart === getCurrentDate()) || (game?.gameDuplicated !== 'YES' && game?.gameGameStage === 'Review' && datePart === getCurrentDate()) || (game?.gameDuplicated !== 'YES' && game?.gameGameStage === 'Launched' && datePart === getCurrentDate())) ? '5px solid #ADD8E6' : 'none'}borderColor={ handelAssign.gameNonPlayingCharacterId===id  ? '#11047a' : ''}> 
...
{game && tabState !== 'charater' && (
  <>
    {game.gameDuplicated === 'YES' && game.gameGameStage === 'Creation' && datePart === getCurrentDate() && (
      <Badge
        position='absolute'
        top='0px'
        left='0px'
    ...
    {game.gameDuplicated !== 'YES' && game.gameGameStage === 'Review' && datePart === getCurrentDate() && (
      <Badge
        position='absolute'
        top='0px'
        left='0px'
    ...
    {game.gameDuplicated !== 'YES' && game.gameGameStage === 'Launched' && datePart === getCurrentDate() && (
      <Badge
        position='absolute'
        top='0px'
        left='0px'
...
{/* dsfdffffffffffffffffffffffffff */}
          {isHovered ? (
            ...
            >
              ...
              >
                <span style={{ color: 'white' }} >{tabState === 'charater' ? (
...
{/* )} */}
  ...
  .map((authorItem, index) => {
    const skillName = tabState === 'All' ? findCatgoryName(authorItem) : findSkillName(authorItem);
    return skillName;
  })
