    ...
    const fetchData = async () => {
      try {
          ...
          console.log("result",result?.data)
           console.log('getCreatorName Error :', result?.message);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (result?.status !== 'Success')
        console.log("result",result.data)
          return console.log('getCreatorName Error :', result?.message);
      } catch (error) {
        console.error('An error occurred while fetching data:', error);
      }
    };
        ...
        <Card alignItems={'center'}>      
          ...
          <Card mb={{ base: '0px', xl: '20px' }} width={{base: '95%', md: '70%'}} marginTop={'-120px'}>
            ...
            <Text color={useColorModeValue('secondaryGray.900', 'white')} fontSize="2xl" fontWeight="700" mb="20px">
                CreatorActivity {id ? 'Updation' : 'Creation'}
              </Text>
