...
export default function LearnerDailyTraffic(props: { [x: string]: any }) {
  const { ...rest } = props;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const dailyData = res?.data?.dailyLearnerCount || [];
  ...
  const barChartDataLearnerDailyTraffic = [
    {
      ...
      data: chartData.map((item) => item.Activecount).reverse(),
    },
  ];
  ...
  const barChartOptionsLearnerDailyTraffic = {
    ...
    xaxis: {
      categories: chartData.map((item) => item.day).reverse(),
      show: false,
      labels: {
        show: true,
        style: {
  ...
  const currentDateExists = chartData.some((item) => item.day === currentDate);

  if (currentDateExists) {
    currentCount = chartData.find((item) => item.day === currentDate).Activecount;
  }
  const yesterdayDateExists = chartData.some((item) => item.day === yesterdayDate);
  if (yesterdayDateExists) {
    yesterdayCount = chartData.find((item) => item.day === yesterdayDate).Activecount;
  }
