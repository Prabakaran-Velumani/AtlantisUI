                ...
                <Box>
                  <Flex className="QuestList">
                    <Icon
                      ...
                      color={'grey'}
                      cursor={'pointer'}
                      onClick={() => deleteQuest(item.gameId, item.gameQuestNo)}
                      className="del-icon"
                    />
                    <Text
                      fontSize="md"
                  ...
                  {Number(item.gameQuestNo) ===
                    Number(progressBlockItems[0].questNo) &&
                    progressBlockItems?.map(
                      (progressItem: any, progressIndex: number) => (
                        <div
                          key={progressIndex}
                          style={{
                            display: 'flex',
                          ...
                          // onMouseOut={(e) => handleMouseOut(e)}
                          onClick={(e) =>
                            handleTargetQuest(progressItem, progressIndex)
                          }
                        >
                          <Icon
                            as={MdDelete}
                  ...
                  {Number(item.gameQuestNo) !==
                    Number(progressBlockItems[0].questNo) &&
                    Object.entries(listBlockItems)
                      .filter(
                        ([key, value]: [string, any]) =>
                          value.questNo === item.gameQuestNo,
                      )
